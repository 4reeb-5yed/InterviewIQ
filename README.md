...
2. **Career Intelligence Report (resume-only)** — a combined **recruiter + ATS + hiring-manager audit**: candidate-stage detection (fair to students/portfolio/personal work), **recruiter simulation** (10s/30s/full review + verdict), **career projection** probabilities (employability, internship, entry-level, interview, startup, enterprise), **true ATS analysis** (parsing, structure, keywords + how an ATS reads it), **section-by-section** review, **project assessment** (judged by category; engineering vs business impact), **market positioning** (realistic/stretch/unlikely roles), employer-grade **gap analysis**, **credibility flags**, and prioritized **before→after ROI**. Every conclusion includes reasoning, evidence found/missing, and a **confidence** level; uses `insufficient_data` instead of guessing; strict anti-inflation scoring; never fabricates metrics.
